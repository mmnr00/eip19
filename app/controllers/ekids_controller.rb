class EkidsController < ApplicationController
	...
	def statekid_new
		...
		ek.update(phs: pars[:phs],descr: pars[:descr], dtp: pars[:dtp], dts: pars[:dts], dte:pars[:dte], tp: pars[:tp], sesd: pars[:sesd], sest: pars[:sest], stat: pars[:stat])
	...
	def ekstat_new
		redirect_to request.referrer(stat: params[:stat])
	end

	#delete once stable
	def ekstat_new_backup
		@ekid = Ekid.find(params[:id])
	...
	def index
		@admin = current_admin
		@phs = ["Permohonan Baru","Permohonan Tidak Lengkap", "Permohonan Dalam Semakan", "Permohonan Ditolak","Permohonan Dalam Senarai Menunggu","Lulus Penilaian","Gagal Penilaian","Kemasukan Program Playschool","Kemasukan Program Intervensi","Kemasukan Program Prasekolah","Tamat Program"]
