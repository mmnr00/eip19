		...
		if params[:sch].present? #start search
			...
			if perse.present? #IC present?
					...
					if perse.ph == params[:ph]
						...
						if params[:prog].present?
							...
							if params[:prog] == "DIDIK ANIS"
								redirect_to new_ddk_path(perse: perse.id) and return
								...
								if prg.tp.include? "SEMINAR ANIS"
									if perse.backg.present?
										...
										flash[:success] = "Pendaftaran Untuk #{prg.name} Berjaya!"
										redirect_to persesch_path(prog: params[:prog], proge: params[:proge]) and return
									else
										flash[:danger] = "Sila Lengkapkan Maklumat Anda"
										redirect_to edit_perse_path(id: perse.id, prog: params[:prog], proge: params[:proge]) and return
									end
								...
								if Perproge.where(perse_id: perse.id, proge_id: params[:proge]).present?
									if Fbproge.where(perse_id: perse.id, proge_id: params[:proge]).present?
										...
										redirect_to newfbproge_path(perse: perse.id, proge: params[:proge]) and return
									end
									
								else
									flash[:danger] = "Anda Tidak Mendaftar ke Program Ini"
									redirect_to persesch_path(proge: params[:proge], prog: params[:prog]) and return
								end
								...
								if params[:regkid].present?
									redirect_to new_ekid_path(perse: perse.id, prog: params[:prog]) and return
								else
									redirect_to ekid_list_path(perse: perse.id) and return
								end
							elsif params[:prog] == "PERSEDIAAN ALAM PEKERJAAN" || params[:prog] == "PENCARIAN PEKERJAAN" || params[:prog] == "DET & JOB COACH" 
								if params[:regkid].present?
									redirect_to new_ilsc_path(perse: perse.id, prog: params[:prog], nwiframe: params[:nwiframe]) and return
								else
									redirect_to ilsc_list_path(perse: perse.id) and return
								end
								...
								if params[:regkid].present?
									...
									redirect_to cpcu_list_path(perse: perse.id) and return
								end

							elsif params[:prog] == "BANTUAN ANIS"
								if params[:regkid].present?
									redirect_to new_btan_path(perse: perse.id, prog: params[:prog]) and return
								else
									redirect_to btan_list_path(perse: perse.id) and return
								end

							elsif params[:prog] == "KAD ANIS" 
								if params[:regkid].present?
									redirect_to new_kdan_path(perse: perse.id, prog: params[:prog], tp: params[:tp]) and return
								else
									redirect_to kdan_list_path(perse: perse.id) and return
								end
							end	

						else
							redirect_to perse_path(id: perse.id, flg: true) and return
						end
				...
				if params[:prog] == "CIKGU ANIS"
					...
					redirect_to new_perse_path(ic: params[:icf], prog: params[:prog], proge: params[:proge], regkid: params[:regkid]) and return
				else
					flash[:notice] = "Sila Lengkapkan Maklumat Anda"
					redirect_to new_perse_path(ic: params[:icf], prog: params[:prog], proge: params[:proge], regkid: params[:regkid], tp: params[:tp]) and return
				end
			end
	
		end
		if params[:nwiframe].present?
			render action: "persesch", layout: "iframe-tmp"
		end
	end
